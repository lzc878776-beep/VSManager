...
namespace VSManager
{
    ...
    public static class PublishScanner
    {
        ...
        /// </summary>
        public static string TermsFile => Path.Combine(
            Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "VSManager", "publish-scan-terms.txt");
    ...
    public sealed class GitHubPublisher
    {
        ...
        public static readonly string[] RequiredIgnores =
        {
            ...
            "*.log", "logs/", "archive/", "Archive/", "*.jsonl"
        };

        public static string LogFile => Path.Combine(
            Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "VSManager", "logs", "publish-" + DateTime.Now.ToString("yyyyMMdd") + ".log");
        ...
        public static string ResolveRepoPath(string configured)
        {
            ...
            try
            {
                ...
                for (int i = 0; dir != null && i < 8; i++, dir = dir.Parent)
                    if (File.Exists(Path.Combine(dir.FullName, "VSManager.csproj"))) return dir.FullName.TrimEnd('\\');
            }
            catch { }
            return "";
        }
