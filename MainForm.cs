...
namespace VSManager
{
	public class MainForm : Form, IRemoteHost, IAgentHost
	{
		...
		private readonly DoubaoVoice _voice;
		...
		private void BuildUi()
		{
			...
			_taskPanel.Bind(_tasks, () => _externals, () => _settings.TaskListClearedAt);
		...
		private async Task<string> DockAllPanes()
		{
			...
			try
			{
				...
				{
					string r = await DteWorker.Run(() => VsService.DockCopilotAsToolWindow(v, _settings.CopilotPaneKeyword));
		...
		private void Header_Paint(object sender, PaintEventArgs e)
		{
			...
			int x = _btnVoice.Left - Dpi.S(14);
			x = DrawChip(g, x, r.Height, $"调试中  {debugging}", debugging > 0 ? Theme.IdleDot : Theme.NoneDot);
			x = DrawChip(g, x - Dpi.S(8), r.Height, $"Copilot 运行中  {busy}", busy > 0 ? Theme.BusyDot : Theme.NoneDot);
			x = DrawChip(g, x - Dpi.S(8), r.Height, $"VS 实例  {list.Count}", Theme.Accent);
			if (_web != null && _settings.WebEnabled)
				DrawChip(g, x - Dpi.S(8), r.Height, !_web.Running ? "手机遥控 未启动" : _web.Active ? "手机遥控 使用中" : "手机遥控 已开启",
					!_web.Running ? Theme.Danger : _web.Active ? Theme.IdleDot : Theme.Accent);
		}

		private static int DrawChip(Graphics g, int right, int headerHeight, string text, Color dot)
		{
		...
		private async void UpdateProfiles(bool quiet)
		{
			...
			try { r = await DteWorker.Run(() => VsService.GetLaunchProfiles(v)); }
			catch { return; }
			_profiles[v.Pid] = r;
		...
		private async void SetProfile(string name)
		{
			...
			string r;
			try { r = await DteWorker.Run(() => VsService.SetLaunchProfile(v, name)); }
		...
		private async Task<string> RunDebug(VsInstance v, string action)
		{
			...
			string r;
			try { r = await DteWorker.Run(() => VsService.DebugAction(v, action)); }
		...
		private async void OnChatSend(string text)
		{
			...
			if (v.Copilot == CopilotState.Busy)
			{
				...
				SetStatus($"「{NameOf(v)}」正忙，已加入任务清单（#{q.Id}，前面 {_tasks.Ahead(q)} 个），空闲后自动发布");
				return;
			}
			...
			if (!r.StartsWith("已发送")) { _pendingSend.Remove(v.Pid); if (v == Selected) { UpdateChatHeader(); _chat.FocusInput(); } }
			else
			{
		...
		private async Task<string> SendChatCore(VsInstance v, string text, IReadOnlyList<ChatImage> images = null)
		{
			...
			if (r.StartsWith("已发送")) _awaitReply[v.Pid] = DateTime.Now.AddSeconds(20);
			SetStatus($"「{NameOf(v)}」{r}" + (r.StartsWith("已发送") ? "" : "　·　右键 VS →「查看发送日志」"));
			UpdateChatHeader();
			_chatSvc.Poke();
			return r;
		}
		...
		Task<string> IAgentHost.QueueTask(VsInstance v, string text) => OnUiAsync(async () =>
		{
			...
			var dup = _tasks.Items.FirstOrDefault(t => t.VsKey == v.Key && QueueStatus.Active(t.Status) && t.Text == text);
			...
			switch (q.Status)
			{
				case QueueStatus.Running:
					return $"已发送（任务 #{q.Id}）到「{name}」。完成后会自动通知你，无需调用 wait_for_vs。";
				case QueueStatus.Waiting:
					return q.Attempts > 0
						? $"发送暂未成功（{q.Error}），任务 #{q.Id} 已留在任务清单，30 秒后自动重试。"
						: $"「{name}」正忙，任务已加入任务清单（#{q.Id}，前面 {_tasks.Ahead(q)} 个），空闲后自动发布，完成后通知你。";
				case QueueStatus.Failed:
					return $"任务 #{q.Id} 发布失败：{q.Error}";
				default:
					return $"任务 #{q.Id}：{StatusText(q)}";
			}
		});
		...
		{
			try { return await DteWorker.Run(() => VsService.ReadErrorList(v, max)).ConfigureAwait(false); }
		...
		private async void RefreshInstances()
		{
			...
			try
			{
				...
				var list = await DteWorker.Run(() => VsService.Enumerate(existing));
		...
		private async void MoveTools(VsInstance v)
		{
			...
			string msg;
			try { msg = await DteWorker.Run(() => VsService.MoveToolWindows(v, area, layout)); }
		...
		private static string StatusText(QueuedTask t)
		{
			switch (t.Status)
			{
				case QueueStatus.Waiting: return t.Attempts > 0 ? "等待重试" : "排队中";
				case QueueStatus.Sending: return "发送中";
				case QueueStatus.Running: return "执行中（" + FormatDuration(DateTime.Now - (t.Started ?? DateTime.Now)) + "）";
		...
		private async void PumpTasks() { try { await PumpTasksAsync(); } catch (Exception ex) { SetStatus("任务清单调度出错：" + ex.Message); } }

		/// <summary>调度任务清单：跟踪执行中的任务，并把每个空闲 VS 最早排队的任务发布出去。只在界面线程调用。</summary>
		private async Task PumpTasksAsync()
		{
			...
			try
			{
				...
				{
					if (now < _tasksReadyAt) break;
					var v = FindVs(t.VsKey);
					if (v == null)
					{
						if ((now - (t.Started ?? now)).TotalSeconds > 15) FailTask(t, "目标 VS 已关闭，无法确认任务结果");
						continue;
					}
					if (v.Copilot == CopilotState.Busy) t.SawBusy = true;
					// 始终没观察到 Copilot 开始运行（任务极快或状态不可读）：按已完成处理
					else if (!t.SawBusy && v.Copilot == CopilotState.Idle && CanDispatch(v) && (now - (t.Started ?? now)).TotalSeconds > 30) FinishTask(t, v, null);
				}

				var heads = _tasks.Items.Where(x => QueueStatus.Active(x.Status)).GroupBy(x => x.VsKey)
				...
				foreach (var t in heads)
				{
					if (_sending) break;
					var v = FindVs(t.VsKey);
					if (v == null || !CanDispatch(v) || t.Status != QueueStatus.Waiting) continue;
					t.Status = QueueStatus.Sending;
					t.VsName = NameOf(v);
					t.Attempts++;
					_tasks.Commit();
					SendLog.Event(t.VsName, $"任务清单：发布任务 #{t.Id}（第 {t.Attempts} 次）");
					string r = await SendChatCore(v, t.Text);
					if (r.StartsWith("已发送"))
					{
						t.Status = QueueStatus.Running;
						t.Started = DateTime.Now;
						t.SawBusy = false;
						t.Error = null;
						_tasks.Commit();
						SetStatus($"任务清单：#{t.Id} 已发布到「{t.VsName}」");
					}
					else if (t.Attempts >= 3) FailTask(t, r);
					else
					{
						t.Status = QueueStatus.Waiting;
						t.Error = r;
						t.NextTry = DateTime.Now.AddSeconds(30);
						_tasks.Commit();
						SetStatus($"任务清单：#{t.Id} 发送失败（{r}），30 秒后重试");
					}
				}
			}
			finally { _pumping = false; }
			_taskTimer.Enabled = _tasks.Items.Any(x => QueueStatus.Active(x.Status));
		}

		private void FailTask(QueuedTask t, string error)
		{
			t.Status = QueueStatus.Failed;
			t.Error = error;
			t.Finished = DateTime.Now;
			_tasks.Commit();
			SetStatus($"任务清单：#{t.Id}「{t.VsName}」失败：{error}");
			if (t.FromAgent)
				_agent.Notify($"📋 任务 #{t.Id} 失败 · {t.VsName}",
					$"[任务完成通知] 任务 #{t.Id} 在「{t.VsName}」发布或执行失败：{error}。任务内容：{Clip(t.Text, 300)}。请判断是否改派、重试或告知用户。");
		}

		/// <summary>任务完成：读取 Copilot 最新回复作为结果，并通知 AI 助手。</summary>
		private async void FinishTask(QueuedTask t, VsInstance v, TimeSpan? dur)
		{
			if (t.Status != QueueStatus.Running) return;
			...
			string answer = TaskSummary.AnswerText(chat);
			t.Result = string.IsNullOrWhiteSpace(answer) ? null : Clip(answer, 1500);
			_tasks.Commit();
			if (t.FromAgent)
			{
				string took = FormatDuration(dur ?? (t.Finished.Value - (t.Started ?? t.Finished.Value)));
				int left = _tasks.Items.Count(x => QueueStatus.Active(x.Status));
				_agent.Notify($"📋 任务 #{t.Id} 已完成 · {t.VsName}（{took}）",
					$"[任务完成通知] 任务 #{t.Id} 已在「{t.VsName}」完成（用时 {took}）。任务：{Clip(t.Text, 300)}\n" +
		...
		private bool CancelQueued(QueuedTask t)
		{
			if (t.Status != QueueStatus.Waiting && t.Status != QueueStatus.Running) return false;
		...
		private void OnTaskAction(QueuedTask t, string action)
		{
			switch (action)
			{
				...
				case "remove": _tasks.Remove(t.Id); return;
				case "retry":
					t.Status = QueueStatus.Waiting;
					t.Attempts = 0;
					t.Error = null;
				...
				case "dispatch":
					t.NextTry = DateTime.MinValue;
					var target = FindVs(t.VsKey);
					if (target == null) SetStatus($"「{t.VsName}」当前未打开，任务会在它打开并空闲后发布");
					else if (!CanDispatch(target) || _tasks.Items.Any(x => x.VsKey == t.VsKey && (x.Status == QueueStatus.Running || x.Status == QueueStatus.Sending)))
						SetStatus($"「{t.VsName}」仍在忙，任务 #{t.Id} 会在空闲后自动发布");
					else if (_tasks.Items.Any(x => x.VsKey == t.VsKey && x.Status == QueueStatus.Waiting && x.Id < t.Id))
						SetStatus($"任务 #{t.Id} 前面还有排队任务，按顺序发布");
					PumpTasks();
					return;
			}
		}
		...
		private async void OnExternalAction(ExternalChat c, string action)
		{
			...
			else if (action == "open")
			{
				...
				try { await DteWorker.Run(() => VsService.OpenCopilotChat(v)); } catch { }
