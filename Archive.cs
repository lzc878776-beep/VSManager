...
namespace VSManager
{
    ...
    public static class Archive
    {
        ...
        public static string AppLogFolder => Path.Combine(Path.GetDirectoryName(AppSettings.FilePath), "logs");
        ...
        private static void Log(string text)
        {
            try
            {
                ...
                File.AppendAllText(Path.Combine(AppLogFolder, "tasks.log"), DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + " [归档] " + text + "\r\n", Encoding.UTF8);
            }
            catch { }
        }
